...
const actions = {
  // 获取用户信息
  async getInfo({ commit }, redirect_uri) {
    try {
      ...
      if (IdmAuthContext.IDM_AUTH.token.state.is_change_pwd) {
        router.replace({ path: '/reset-password', query: { redirect_uri: redirect_uri || encodeURIComponent(location.href) } });
        return false;
      }
    } catch (error) {
      console.log(error);
  ...
  setRealName({commit}, name) {
    commit('SET_USERINFO', { realName: name })
  }
}
